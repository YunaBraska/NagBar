...
| Test target runs | Done | Full suite passes; current suite is 324 tests |
| Shared scheme exists for CI/CLI | Done | `NagBar.xcodeproj/xcshareddata/xcschemes/NagBar.xcscheme` |
| Runtime smoke launches release app | Done | Process confirmed; status-item, status-panel, About, Preferences, Refresh, and Quit paths are verified without publishing screenshots |
...
| PromiseKit | Done | Replaced by app-local `Promise` compatibility layer; no `PromiseKit` references remain, focused async/fake-server tests pass, full suite passes 324 tests, Release build passes |
| hpple | Done | Replaced by app-local tolerant HTML row parser for Nagios/Icinga CGI tables; no Hpple/hpple/TFHpple/XPath references remain, 23 focused Nagios/Icinga parser tests pass, full suite passes 324 tests, Release build passes |
| RealmSwift active storage | Done | Scalar settings moved to `UserDefaults`; filters, server login settings, and monitoring instances moved to Application Support JSON; focused monitoring suite passes 56 tests, full suite passes 324 tests, Release build passes |
...
| Production readiness coverage gate | In progress | `NagBar.app` coverage is 83.17% from Xcode result metrics after 324 passing tests; `StatusBarAnimationTrigger.swift`, `CommandsTabController.swift`, `PasswordPromptController.swift`, `PasswordPromptFlow.swift`, `FilterTableViewDatasource.swift`, `FilterTableViewDelegate.swift`, `MenuAction.swift`, and `MonitoringInstancesTabController.swift` are 100% covered, `Icinga2Commands.swift` is 98.55%, `FilterOptionsTabController.swift` is 91.67%, `MonitoringItemSorter.swift` is 90.07%, `InterfaceAction.swift` is 80.17%, `StatusPanelTableDelegate.swift` is 71.70%, and `AudibleAlarmsTabController.swift` is 55.00%; remaining high-yield gaps are AppKit-heavy `StatusBar.swift`, `StatusItemView.swift`, `AppDelegate.swift`, `AnimateStatusBar.swift`, and drawing/window paths |
...
| Item | Status | Acceptance gate |
| --- | --- | --- |
| First-run setup path | Done | New user starts against a local fake Icinga HTTP server when no real backend exists, can open Settings from the status item, and can replace the fallback by saving a real Icinga-compatible remote without reading docs; replacement is covered through the same AppKit table controls/delegates used by the Settings editor, and live smoke proves isolated status-item Preferences reachability |
| Single status-item entrypoint | Done | Status-item menu exposes Show Status, About, Preferences, Refresh, and Quit with stable accessibility identifiers; status-item refresh uses an injected entrypoint; status-panel opening guards missing first-refresh data; Release smoke opens the live status-item menu with AXPress, verifies Show Status opens an onscreen status panel, verifies keyboard Down/Return activates Show Status, verifies Refresh keeps the status item alive, and quits through the same menu. Global keyboard focus into macOS menu extras is system-setting dependent and remains a manual accessibility check |
